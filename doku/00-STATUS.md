...
status: teilweise
fortschritt: 70
zusammenfassung: Gesamtstand 76,5 „Solide“ (04.09.2026). Verbesserungslauf 4, SU04 und GE23/GE25/IS19 liegen auf origin/main; drei belegte Wissensbeiträge, Antwortabsätze mit belegten Zahlen, keine dünne Seite mehr. PJ05 (kritische Audit-Funde abgearbeitet oder begründet), SI08 (CSP blockiert) und PJ11 (Paketfassungen samt Lockfile) sind seit dem Merge 0c18ea7 auf main und origin/main; SI09 (Nonce statt 'unsafe-inline' im script-src) liegt auf dem Zweig sofort/2026-09-11-si09, nicht gemergt und im Browser ungeprüft. TS11 wartet weiter auf CANONICAL_HOST. Die drei ersten Beiträge und die Produktbeschreibungen warten auf die Betreiberin.
offen: 3
...
| **Ordner-Stand** | Zweig `sofort/2026-09-11-si09` @ `28d1ca3` (11.09.2026), ein Commit vor `main`, nicht gemergt. `sofort/2026-09-11-pj05-und-2-weitere` ist mit `0c18ea7` in `main` gemergt; `main` = `origin/main` = `bc36dd6`. `cockpit/2026-09-01-verbesserung-4` (`511ffe5`) ist in `origin/main` enthalten — die Zeile „Live-Stand" darüber ist damit überholt (siehe eigener Punkt `EIG11` in [80-AUFGABEN.md](80-AUFGABEN.md)) |
...
| Aufgaben | teilweise | 40 | SU04, GE23, GE25, IS19 und seit dem Merge `0c18ea7` auch PJ05, SI08 und PJ11 liegen auf main und origin/main (Auslieferung durch Railway nicht geprüft). SI09 ist am 11.09.2026 auf dem Zweig `sofort/2026-09-11-si09` erledigt, nicht gemergt und im Browser ungeprüft; `'unsafe-eval'` für Alpine.js und `'unsafe-inline'` im `style-src` bleiben. Nächster Schritt ist der Merge, danach Bezahlseite und Ersatzskripte mit Browserkonsole und der erste CI-Lauf mit Django 5.2.17; Schriften lokal, Sprungmarke, Permissions-Policy, Danke-Seite und die Freigaben der Betreiberin bleiben offen. | [80-AUFGABEN.md](80-AUFGABEN.md) |
...
1. **Den Zweig `sofort/2026-09-11-si09` nach `main` bringen und pushen, in Railway `CANONICAL_HOST=www.luviq-alsfeld.com` setzen.** Die drei Commits vom 11.09.2026 (durchgesetzte CSP, feste Paketfassungen, Subscriber im Django-Admin) sind mit `0c18ea7` bereits auf `main` und `origin/main`; offen sind `28d1ca3` (SI09, Nonce statt `'unsafe-inline'`) und die 301 für `luviq-alsfeld.com` (TS11), deren Vorgabe auch in diesem Zweig leer ist. Danach mit offener Browserkonsole die Bezahlseite `/payment/<id>/` und die Ersatzskripte für die früheren `on…`-Attribute (Löschrückfragen, Bestellfilter, Kampagnenknöpfe) ansehen und den ersten CI-Lauf mit Django 5.2.17 abwarten — beides ist bisher nicht belegt.
...
| Datum | Was | Beleg |
|---|---|---|
| 11.09.2026 | `SI09` auf `sofort/2026-09-11-si09` (nicht gemergt): `script-src` ohne `'unsafe-inline'`, Nonce je Anfrage an jedem Inline-Skript und am PayPal-SDK, 17 Handler-Attribute durch `data-`-Attribute ersetzt, drei neue Tests; kein Element, keine Klasse, keine Kennung verändert, 218/218 Tests grün laut Commit. Offen: Verhalten der Ersatzskripte im Browser, `'unsafe-eval'` (Alpine.js) und `'unsafe-inline'` im `style-src` | `28d1ca3` |
