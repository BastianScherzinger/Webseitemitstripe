...
| **Ordner-Stand** | Zweig `sofort/2026-09-12-si17-und-2-weitere` @ `11acccc` (12.09.2026), zwei Commits vor `main` (`680a641` SI17 — `integrity`/`crossorigin` an den vier Fremdskripten; `11acccc` PJ01 — zweiter Prüfbefehl `pruefe_links`), nicht gemergt und in keinem Remote-Zweig. `sofort/2026-09-12-vl21-und-2-weitere` ist mit `0d5500b`, `sofort/2026-09-11-kv07-und-2-weitere` mit `c522ff9`, `sofort/2026-09-11-is18` mit `1a5f36b`, `sofort/2026-09-11-si09` mit `4ec540b`, `sofort/2026-09-11-pj05-und-2-weitere` mit `0c18ea7` in `main` gemergt; `main` = `7f5ef63`, `origin/main` = `f90d791` — **`main` ist vier Doku-Commits voraus, der Push steht aus** (lokale Referenz, geprüft 12.09.2026). `sofort/2026-09-12-re07-und-2-weitere` (Paket 188) ist nicht gemergt, unterscheidet sich von `main` aber nur in `LOGBUCH.md` und `doku/`. `cockpit/2026-09-01-verbesserung-4` (`511ffe5`) ist in `origin/main` enthalten — die Zeile „Live-Stand" darüber ist damit überholt (siehe eigener Punkt `EIG11` in [80-AUFGABEN.md](80-AUFGABEN.md)) |
...
| Technik | teilweise | 74 | Stack läuft stabil. Seit den Merges `0c18ea7`, `4ec540b`, `1a5f36b`, `c522ff9` und `0d5500b` liegen auf main die scharf gestellte CSP, die Nonce im `script-src` statt `'unsafe-inline'`, die festgenagelten Paketfassungen samt `requirements.lock` (Django 5.2.17), die abgearbeiteten Audit-Funde, die Danke-Seite `/kontakt/danke/` und die Produktkarte als zwei Bausteine unter `shop1/templates/shop1/teile/` (neue Falle: `load` wird an ein `include` nicht vererbt); `main` = `7f5ef63`, vier Doku-Commits vor `origin/main`. Im Zweig `sofort/2026-09-12-si17-und-2-weitere` (zwei Commits, nicht gemergt) tragen die vier Fremdskripte von jsDelivr `integrity` und `crossorigin`, und neben `pruefe_seite` steht der zweite Prüfbefehl `pruefe_links` (jeder Verweis jeder öffentlichen Seite, 230 Tests grün, nicht an `start.sh` angeschlossen). Offen bleiben der Merge, Bezahlseite und Ersatzskripte mit Browserkonsole, die zwei Chart.js-Einbindungen des Admin-Panels ohne `integrity`, `'unsafe-eval'` für Alpine.js, der erste CI-Lauf mit 5.2.17, `CANONICAL_HOST` in Railway, `runtime.txt`/`railway.json` und die Permissions-Policy. | [10-TECHNIK.md](10-TECHNIK.md) |
...
1. **Den Zweig `sofort/2026-09-12-si17-und-2-weitere` nach `main` bringen und `main` pushen (es liegen bereits vier Doku-Commits vor `origin/main`), in Railway `CANONICAL_HOST=www.luviq-alsfeld.com` setzen.** Durchgesetzte CSP, feste Paketfassungen, Subscriber im Django-Admin (`0c18ea7`), SI09 (`4ec540b`, Nonce statt `'unsafe-inline'`), IS18 (`1a5f36b`, längere belegte Texte auf `/`, `/produkte/` und `/wissen/` — nachmessen) und KV07 (`c522ff9`, Danke-Seite `/kontakt/danke/` — nach dem Deploy einmal das Formular abschicken) und VL21 (`0d5500b`, Angebotsbausteine unter `shop1/templates/shop1/teile/` — ändert am gerenderten HTML nichts) sind bereits auf `main` (`7f5ef63`), ob Railway sie ausgeliefert hat, ist nicht geprüft; offen sind `680a641` (SI17, `integrity` an den vier Fremdskripten), `11acccc` (PJ01, Prüfbefehl `pruefe_links`) und die 301 für `luviq-alsfeld.com` (TS11), deren Vorgabe weiter leer ist. Danach mit offener Browserkonsole die Bezahlseite `/payment/<id>/` und die Ersatzskripte für die früheren `on…`-Attribute (Löschrückfragen, Bestellfilter, Kampagnenknöpfe) ansehen und den ersten CI-Lauf mit Django 5.2.17 abwarten — beides ist bisher nicht belegt.
