...
status: teilweise
fortschritt: 70
zusammenfassung: Gesamtstand 76,5 „Solide“ (04.09.2026). Verbesserungslauf 4, SU04 und GE23/GE25/IS19 liegen auf origin/main; drei belegte Wissensbeiträge, Antwortabsätze mit belegten Zahlen, keine dünne Seite mehr. PJ05 (kritische Audit-Funde abgearbeitet oder begründet), SI08 (CSP blockiert) und PJ11 (Paketfassungen samt Lockfile) sind seit dem Merge 0c18ea7, SI09 (Nonce statt 'unsafe-inline' im script-src, im Browser ungeprüft) seit 4ec540b und IS18 (Startseite, Produktübersicht und Wissensübersicht laut Commit im Umfang ihrer Seitenart) seit 1a5f36b auf main und origin/main. Auf dem Zweig sofort/2026-09-11-kv07-und-2-weitere (nicht gemergt) leitet das Kontaktformular auf die eigene Danke-Seite /kontakt/danke/ (KV07); VL01 ist als Ausnahme „bewusst so“ begründet, GE11 wartet auf Profiladressen der Betreiberin. TS11 wartet weiter auf CANONICAL_HOST. Die drei ersten Beiträge und die Produktbeschreibungen warten auf die Betreiberin.
offen: 3
...
| **Ordner-Stand** | Zweig `sofort/2026-09-11-kv07-und-2-weitere` @ `7919341` (11.09.2026), zwei Commits vor `main` (`2d78a55` KV07, `7919341` VL01 — nur Doku), nicht gemergt und in keinem Remote-Zweig. `sofort/2026-09-11-is18` ist mit `1a5f36b`, `sofort/2026-09-11-si09` mit `4ec540b`, `sofort/2026-09-11-pj05-und-2-weitere` mit `0c18ea7` in `main` gemergt; `main` = `origin/main` = `5722d70` (lokale Referenz, geprüft 11.09.2026). `cockpit/2026-09-01-verbesserung-4` (`511ffe5`) ist in `origin/main` enthalten — die Zeile „Live-Stand" darüber ist damit überholt (siehe eigener Punkt `EIG11` in [80-AUFGABEN.md](80-AUFGABEN.md)) |
...
| Aufgaben | teilweise | 50 | SU04, GE23, GE25, IS19, PJ05, SI08, PJ11, SI09 und seit dem Merge `1a5f36b` auch IS18 liegen auf main und origin/main (lokale Referenz; ob Railway ausgeliefert hat, ist nicht geprüft; SI09 im Browser ungeprüft). Paket 171 (11.09.2026, Zweig `sofort/2026-09-11-kv07-und-2-weitere`, nicht gemergt) bringt KV07: das Kontaktformular leitet nach dem Absenden auf die eigene Danke-Seite `/kontakt/danke/` (noindex, nicht in Sitemap und llms.txt, never_cache), bestehende Seiten unverändert, 224/224 Tests grün laut Commit. VL01 ist als Ausnahme „bewusst so“ im Bewertungsblock begründet, kein Umbau; GE11 ist nicht möglich, bis die Betreiberin weitere Profiladressen nennt. Nächster Schritt ist der Merge dieses Zweigs, danach Nachmessung, Bezahlseite und Ersatzskripte mit offener Browserkonsole und der erste CI-Lauf mit Django 5.2.17; Schriften lokal, Sprungmarke, Icon-Knöpfe, Permissions-Policy und die Freigaben der Betreiberin bleiben offen. | [80-AUFGABEN.md](80-AUFGABEN.md) |
...
1. **Den Zweig `sofort/2026-09-11-kv07-und-2-weitere` nach `main` bringen und pushen, in Railway `CANONICAL_HOST=www.luviq-alsfeld.com` setzen.** Durchgesetzte CSP, feste Paketfassungen, Subscriber im Django-Admin (`0c18ea7`), SI09 (`4ec540b`, Nonce statt `'unsafe-inline'`) und IS18 (`1a5f36b`, längere belegte Texte auf `/`, `/produkte/` und `/wissen/` — nachmessen) sind bereits auf `main` und `origin/main`, ob Railway sie ausgeliefert hat, ist nicht geprüft; offen sind `2d78a55` (KV07, Danke-Seite `/kontakt/danke/` — nach dem Deploy einmal das Formular abschicken) und die 301 für `luviq-alsfeld.com` (TS11), deren Vorgabe weiter leer ist. Danach mit offener Browserkonsole die Bezahlseite `/payment/<id>/` und die Ersatzskripte für die früheren `on…`-Attribute (Löschrückfragen, Bestellfilter, Kampagnenknöpfe) ansehen und den ersten CI-Lauf mit Django 5.2.17 abwarten — beides ist bisher nicht belegt.
