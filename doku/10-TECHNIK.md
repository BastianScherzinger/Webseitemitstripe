...
| Mail | `USE_SMTP_EMAIL`, `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_HOST_USER`, `EMAIL_HOST_PASSWORD`, `DEFAULT_FROM_EMAIL` (ohne Variable `noreply@luviq-alsfeld.com` seit `MW22`, `ed835a9`, Zweig nicht gemergt — vorher `noreply@luviq-shop.de`, eine Domain, die es nicht gibt; Brevo verschickt davon erst, wenn `luviq-alsfeld.com` im Brevo-Konto bestätigt ist), `BREVO_API_KEY` |
...
| **Verkauf (Zweig `recht/2026-09-18-marke-im-aufbau`)** | `VERKAUF_AKTIV` — **Vorgabe aus** (`1/true/yes/on/ja/an` schaltet ein). Aus = Marke im Aufbau: keine Preise, kein Warenkorb, keine Kasse, kein PayPal, keine Sätze zu § 19 UStG/Zahlung/Versand, `Organization` statt `Offer`/`LocalBusiness`, AGB `noindex` und aus Sitemap/llms.txt, Kaufweg-Wissensbeiträge leiten mit 302 auf `/wissen/`; die Stücke erscheinen als Archiv bisheriger, bereits vergebener Stücke („Nº 00x · Archiv"). Siehe Falle 22 und 23. `BETREIBER_KONTEN` (Komma-Liste, Vorgabe `luisabre`): Gästebuch-Beiträge dieser Konten erscheinen nicht auf der Startseite |
| **Nur Zweig 18.09. (Paket 306)** | `INDEXNOW_KEY` — Schlüssel für IndexNow (`shop1/indexnow.py`), 8–128 Zeichen aus Buchstaben, Ziffern und Bindestrich; leer oder ungültig = aus (Vorgabe). Gemeldet wird unter dem Host von `SITE_URL` — `SITE_URL` muss also auf `https://www.luviq-alsfeld.com/` stehen (seit 16.09.2026 so gesetzt, [80-AUFGABEN.md](80-AUFGABEN.md) „Offen" Nr. 0) |
...
│   ├── indexnow.py     IndexNow-Meldungen für Produktänderungen (Zweig 18.09., Paket 306; aus ohne INDEXNOW_KEY)
...
| Module ohne eigenen Test — von den zehn gemeldeten werden seit Paket 306 (Zweig 18.09., `156176e`) sieben direkt eingeführt (`forms.py`, `signals.py`, `views/auth.py`, `views/cart.py`, `views/checkout.py`, `views/gaestebuch.py`, `pruefe_seite.py`); offen bleiben `views/_helpers.py`, `views/shop.py` und `tiktok stream/streamtest.py` (Altablage). Die View-Module sind über Seiten-, Konto- und Zahlungstests indirekt abgedeckt, das Audit zählt nur direkte Importe | PJ03 | PJ03 |
| Zweig `sofort/2026-09-18-pj06-und-2-weitere` nach `main` mergen und pushen, `INDEXNOW_KEY` in Railway setzen, danach `/indexnow-schluessel.txt` abrufen und nach dem Speichern eines Stücks das Railway-Log auf „IndexNow lehnt die Meldung ab" / „IndexNow nicht erreichbar" ansehen | `0b94d8e` — `IndexNowTest` ersetzt den Pool durch eine Attrappe, ein echter Aufruf an `api.indexnow.org` hat nie stattgefunden | PJ13 |
| `CustomUserCreationForm.save()` setzt `land` bei leerem Feld auf `''` statt „Deutschland" — `self.cleaned_data.get('land', 'Deutschland')` (`shop1/forms.py:123`) greift nur, wenn der Schlüssel fehlt | beim Schreiben von `KontoBausteineTest` aufgefallen (`LOGBUCH.md`, Paket 306), nicht angefasst | — |
